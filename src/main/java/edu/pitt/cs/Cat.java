	...
	public static Cat createInstance(InstanceType type, int id, String name) {
		switch (type) {
			...
			case MOCK:
				...
				Mockito.when(mockCat.toString()).thenReturn(String.format("ID %d. %s", id, name));
				// Mockito by default returns false on boolean methods, no need to mock getRented() method
				return mockCat;
			default:
				assert (false);
				return null;
